  ...
  if (userId.value !== undefined) {
    userService.getUserById(userId.value as string).then(async (response) => {
      ...
      if (err.response.status === 403) {
        toast.info("Vous n'avez pas les permissions requises pour faire ça");
...
<template>
  ...
  <div v-else>
    ...
    <div class="not-found" v-if="workingTimes !== null && workingTimes.length <= 0 && !loading">
      <h1>{{user?.firstname ?? auth?.user?.firstname}} {{user?.lastname?.toUpperCase() ?? auth?.user?.lastname?.toUpperCase()}} n'a pas de temps de travail</h1>
