...
const loadWorkingTimes = () => {
  ...
  if (auth.isLogged === IsLogged.Logged && auth.user !== undefined) {
    workingTimeService.getWorkingTimes(auth.user?.id!, `${now.toFormat('yyyy-MM-dd')} 00:00:00`, `${now.plus({day: 1}).toFormat('yyyy-MM-dd')} 00:00:00`).then((response) => {
      dailyWorkingTimes.value = response as IWorkingTime[];
    });
    workingTimeService.getWorkingTimes(auth.user?.id!, `${weekStart.toFormat('yyyy-MM-dd')} 00:00:00`, `${weekEnd.toFormat('yyyy-MM-dd')} 00:00:00`).then((response) => {
  ...
  <v-layout>
    <v-row
      ...
      <v-col lg="12" sm="12" md="12" cols="12">
        <WeeklyChart :times="weeklyWorkingTimes" :on-picker-change="onWeekChange"/>
      </v-col>
      <v-col lg="4" cols="4">
        <DailyChart :times="dailyWorkingTimes"/>
      </v-col>
      <v-col lg="8" cols="8">
        <MonthlyChart :times="monthlyWorkingTimes" :on-picker-change="onMonthChange"/>
      </v-col>
    </v-row>
  </v-layout>
</template>
