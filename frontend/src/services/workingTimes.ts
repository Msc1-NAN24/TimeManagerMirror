...
const getTeamWorkingTimes = (id: string, callback: (workingTimes: IWorkingTime[]) => void) => {
    return workingTimeRepository.getTeamWorkingTime(id, callback);
}

const getTeamWorkingTimesByPeriod = (id: string, start: string, end: string, callback: (workingTimes: IWorkingTime[]) => void) => {
    return workingTimeRepository.getTeamWorkingTimeByPeriod(id, start, end, callback);
}
